...
import { getBoardPage, getAllBids } from "@/lib/db";
...
export default async function Home({
  ...
  // Default tab is "handle" (X); fetch its first page + all bids for the form.
  const [initialPage, allBids] = await Promise.all([
    getBoardPage({ tab: "handle", page: 1 }),
    getAllBids(),
  ]);
